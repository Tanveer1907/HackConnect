...
1. **Admin access (A3):** ~~add a real admin login to the React app, or keep admin in EJS?~~ **DECIDED (2026-07-06): keep admin access** — add a React admin-login flow that sets the `admin_token` cookie so [AdminModeration.jsx](frontend/src/pages/AdminModeration.jsx) works (rather than dropping the page).
2. **Learning/certification card & badges:** ~~build a real feature or remove?~~ **DONE (2026-07-06): removed** — deleted the "Full Stack Certification" card from [Dashboard.jsx](frontend/src/pages/Dashboard.jsx) and the hardcoded Badges block from [Profile.jsx](frontend/src/pages/Profile.jsx).
3. **Animation library:** ~~stay with CSS keyframes, or introduce framer-motion?~~ **DECIDED (2026-07-06): add `framer-motion`** for page/list/exit transitions. It installs as a normal npm dependency into the existing CRA build (no tooling change). Use it for the richer B2 transitions (page transitions, list stagger, exit animations); keep simple one-shot entrances (B0) as CSS keyframes registered in the inline Tailwind config in `index.html`.
